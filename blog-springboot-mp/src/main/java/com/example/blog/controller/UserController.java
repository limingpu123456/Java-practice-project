...
import com.example.blog.util.AppVar;
...
public class UserController {
    ...
    public AjaxResult savePhoto(MultipartFile file,HttpServletRequest request){
        ...
        //生成图片的名称
        String imgName = UUID.randomUUID().toString().replace("-","")+imageType;
        try {
        ...
        if(result){
            ...
            session.setAttribute(AppVar.SESSION_KEY_USERINFO,userInfo);
            return AjaxResult.succ("/image/"+imgName);
        }
        return AjaxResult.fail(-3,"数据库返回失败");
    }
    ...
    public AjaxResult update(String nickname, String oldpassword,
                                String password, Boolean isUpdatePassword,HttpServletRequest request){
        //1.参数校验
